...
### Valutazione tecnica del secondo incremento

| Candidato | API ufficiale / documento e path | Associazione foreground e multi-istanza | Dipendenze / fallback | Fake test / privacy | Manutenzione | Esito |
|---|---|---|---|---|---|---|
| AutoCAD | .NET/COM: documento MDI attivo e filename | Solida in add-in; fragile da adapter esterno e ambigua tra istanze | Add-in preferibile; niente title parsing | Facade sì; path sensibile | Alta per versioni/deployment | Rinviato |
| Autodesk Inventor | Automation/COM: `ActiveDocument.FullFileName` | HWND e istanza foreground meno deterministici da ROT | COM; add-in più robusto | Facade sì; path sensibile | Medio-alta | Rinviato |
| Revit | API .NET: active UI document e `PathName` | Affidabile soprattutto in-process; accesso esterno non equivalente | Add-in necessario; niente UIA/title | Facade sì in add-in; modello sensibile | Alta | Rinviato |
| Microsoft Excel | Object Model/COM: `ActiveWorkbook.FullName`/`Name`, `Application.Hwnd` | Match HWND+PID conservativo; altra istanza ROT => `Unresolved` | COM; nessun add-in/UIA/title | Facade semplice; nessun contenuto | Media, API stabile | **Scelto** |
| Adobe Acrobat/Reader | Interapplication API limitata | Documento/path foreground non affidabile per Reader e più istanze | Integrazione/add-in o UIA; title vietato | Contratto fake debole; PDF sensibile | Alta | Scartato per ora |

Excel prevale per affidabilità dell'API documentata, controllo foreground conservativo, privacy e testabilità. AutoCAD avrebbe priorità a parità tecnica, ma senza add-in l'associazione esterna corretta è più fragile; introdurre un add-in aumenterebbe deployment e manutenzione.
...
## Secondo incremento implementato

- Sostituito il gate globale con gate per istanza resolver: serializzazione per resolver, indipendenza tra resolver e creazione del task solo dopo l'acquisizione bounded.
- Aggiunto Microsoft Excel come secondo resolver Tier 1 tramite Object Model/COM e facade minima fake-testable. PID e HWND foreground devono coincidere con la finestra dell'applicazione; mismatch e ambiguità multi-istanza producono `Unresolved`.
- Excel legge soltanto `ActiveWorkbook.FullName` e `Name`; non acquisisce window title o contenuto. Riusa timeout, metriche, dispatcher, failure taxonomy, precision/provenance e schema v3, senza migration v4.
- Automated validation: **PASS**. Real Windows application validation: **PENDING** per Word ed Excel installati, scenari multi-istanza, Protected View, access denied e chiusura durante la risoluzione.
- Aggiunti 10 test Windows (totale suite: 136) per serializzazione per-resolver, indipendenza, timeout isolati, cancellation, bounded work e contratto Excel.
- Sprint 02B resta **APERTO**: recovery/profiling e validazione applicativa reale non sono completati.
