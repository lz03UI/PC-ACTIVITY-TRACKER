...
public sealed class TrackingStateMachineTests
{
    ...
    [Fact] public void WallClockGoingBackwardKeepsMonotonicElapsed() { var m = Started(); m.Apply(new(TrackingSignalKind.Reconcile, new(Origin.AddHours(1)), new(1), A)); var effects = m.Apply(new(TrackingSignalKind.ForegroundChanged, new(Origin), new(2), B)); var interval = Assert.IsType<IntervalClosed>(effects[0]).Interval; Assert.Equal(TimeSpan.Zero, interval.Period.Duration); Assert.Equal(TimeSpan.FromSeconds(1), interval.Elapsed); Assert.True(interval.IsElapsedMonotonic); }
    [Fact] public void WallClockJumpingForwardDoesNotInflateElapsed() { var m = Started(); m.Apply(new(TrackingSignalKind.Reconcile, new(Origin), new(1), A)); var interval = Assert.IsType<IntervalClosed>(m.Apply(new(TrackingSignalKind.ForegroundChanged, new(Origin.AddHours(5)), new(2), B))[0]).Interval; Assert.Equal(TimeSpan.FromHours(5), interval.Period.Duration); Assert.Equal(TimeSpan.FromSeconds(1), interval.Elapsed); }
    [Fact] public void NormalWallAndMonotonicAdvanceAgree() { var m = Active(); m.Apply(Signal(TrackingSignalKind.Pause, 2)); var gap = Assert.IsType<GapClosed>(m.Apply(Signal(TrackingSignalKind.Resume, 12))[0]).Gap; Assert.Equal(TimeSpan.FromSeconds(10), gap.Period.Duration); Assert.Equal(TimeSpan.FromSeconds(10), gap.Elapsed); Assert.True(gap.IsElapsedMonotonic); }
