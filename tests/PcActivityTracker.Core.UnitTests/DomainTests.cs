...
public sealed class DomainTests
{
    ...
    [Fact] public void DocumentResolutionUsesExplicitStatesAndProvenance() { var full = DocumentResolutionResult.FullPath(@"C:\Work\plan.docx", DocumentProvenance.DirectlyObserved, "word"); var name = DocumentResolutionResult.FileNameOnly("plan.docx", DocumentProvenance.Derived, "fallback"); var missing = DocumentResolutionResult.Unresolved(DocumentResolutionFailure.TimedOut, "word"); Assert.Equal(DocumentResolutionPrecision.FullPath, full.Precision); Assert.Equal(DocumentResolutionPrecision.FileNameOnly, name.Precision); Assert.Equal(DocumentProvenance.Derived, name.Provenance); Assert.Null(missing.Value); Assert.Equal(DocumentProvenance.Unresolved, missing.Provenance); }
