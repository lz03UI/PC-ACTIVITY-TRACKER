...
public sealed class SqliteActivityStore(SqliteDatabase database) : IObservationStore, IClassificationStore, IPrivacyStore, IWorkTaxonomyStore, IRetentionStore
{
    ...
    public async Task<IReadOnlyList<ActivityInterval>> GetActivityIntervalsAsync(TimeRange period, CancellationToken cancellationToken = default) => await QueryAsync<ActivityInterval>("SELECT * FROM activity_intervals WHERE start_utc < $end AND end_utc > $start ORDER BY start_utc,id;", c => { Add(c, "$start", Format(period.Start)); Add(c, "$end", Format(period.End)); }, r => new(new(Guid.Parse(r.GetString("id"))), new(Guid.Parse(r.GetString("observation_id"))), new(Parse(r.GetString("start_utc")), Parse(r.GetString("end_utc"))), (ActivityState)r.GetInt32("state"), (DiscontinuityReason)r.GetInt32("end_reason")), cancellationToken);
    ...
    public async Task<int> DeleteActivityBeforeAsync(UtcInstant cutoff, CancellationToken cancellationToken = default) => await ExecuteAsync("DELETE FROM observations WHERE observed_at_utc < $cutoff;", c => Add(c, "$cutoff", Format(cutoff)), cancellationToken);
