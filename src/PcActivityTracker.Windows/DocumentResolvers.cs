...
public sealed class DocumentResolverRegistry
{
    ...
    private readonly IReadOnlyDictionary<IDocumentContextResolver, SemaphoreSlim> isolationGates;
        ...
        DocumentResolverMetrics? metrics = null, TimeProvider? timeProvider = null)
    {
        ...
        isolationGates = entries.Values.Distinct(ResolverReferenceComparer.Instance)
            .ToDictionary(static resolver => resolver, static _ => new SemaphoreSlim(1, 1), ResolverReferenceComparer.Instance);
        timeout = (options ?? new()).Timeout;
    ...
    public async ValueTask<DocumentResolutionResult> ResolveAsync(DocumentResolutionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var isolationGate = isolationGates[resolver];
    ...
    private sealed class ResolverReferenceComparer : IEqualityComparer<IDocumentContextResolver>
    {
        public static ResolverReferenceComparer Instance { get; } = new();
        public bool Equals(IDocumentContextResolver? x, IDocumentContextResolver? y) => ReferenceEquals(x, y);
        public int GetHashCode(IDocumentContextResolver obj) => RuntimeHelpers.GetHashCode(obj);
    }
}
...
public interface IExcelDocumentFacade
{
    ExcelDocumentSnapshot? TryReadActiveWorkbook(DocumentResolutionRequest request);
}

public sealed record ExcelDocumentSnapshot(string? FullName, string? Name);

public sealed class ExcelDocumentContextResolver(IExcelDocumentFacade facade) : IDocumentContextResolver
{
    public string Id => "microsoft-excel-com-v1";
    public IReadOnlySet<string> SupportedProcessNames { get; } =
        new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "EXCEL" };

    public ValueTask<DocumentResolutionResult> ResolveAsync(DocumentResolutionRequest request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var workbook = facade.TryReadActiveWorkbook(request);
        cancellationToken.ThrowIfCancellationRequested();
        if (workbook?.FullName is { } fullName && IsFullyQualifiedWindowsPath(fullName))
            return ValueTask.FromResult(DocumentResolutionResult.FullPath(fullName, DocumentProvenance.DirectlyObserved, Id));
        if (workbook?.Name is { } name && !string.IsNullOrWhiteSpace(name))
            return ValueTask.FromResult(DocumentResolutionResult.FileNameOnly(Path.GetFileName(name), DocumentProvenance.DirectlyObserved, Id));
        return ValueTask.FromResult(DocumentResolutionResult.Unresolved(DocumentResolutionFailure.None, Id));
    }
...
/// <summary>Adapter minimale all'Excel Object Model: legge solo Hwnd, FullName e Name del workbook attivo.</summary>
public sealed partial class ExcelComDocumentFacade : IExcelDocumentFacade
{
    public ExcelDocumentSnapshot? TryReadActiveWorkbook(DocumentResolutionRequest request)
    {
        if (request.WindowHandle == 0 || !WindowBelongsToProcess(request.WindowHandle, request.ProcessId)) return null;
        object? application = null, workbook = null;
        try
        {
            ThrowIfFailed(CLSIDFromProgID("Excel.Application", out var classId));
            ThrowIfFailed(GetActiveObject(in classId, 0, out application));
            if (application is null || Convert.ToInt64(GetProperty(application, "Hwnd")) != request.WindowHandle.ToInt64()) return null;
            workbook = GetProperty(application, "ActiveWorkbook");
            if (workbook is null) return null;
            return new(GetProperty(workbook, "FullName") as string, GetProperty(workbook, "Name") as string);
        }
        finally
        {
            Release(workbook); Release(application);
        }
    }

    private static bool WindowBelongsToProcess(nint window, int expectedProcessId)
    {
        _ = GetWindowThreadProcessId(window, out var actualProcessId);
